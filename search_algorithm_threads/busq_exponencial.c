/*
IMPLEMENTACION DEL ALGORITMO DE BUSQUEDA EXPONENCIAL PARALELA (CON HILOS)
Autores: 
        Garcia Peñalva Saul 
...
DESCRIPCION: Este codigo implementa la Búsqueda Exponencial sobre un arreglo previamente 
ordenado con QuickSort, paralelizando el proceso mediante la biblioteca Pthreads. El arreglo 
se divide en bloques equitativos (chunks) asignados a cada hilo. Los hilos realizan una 
verificación temprana para saber si el objetivo puede existir en su rango. De ser así, 
el hilo ejecuta saltos exponenciales (1, 2, 4, 8...) de manera local a partir del inicio 
de su bloque para acotar el subrango, y finalmente remata con una búsqueda binaria interna. 
Si el objetivo no pertenece a su bloque, el hilo muere instantáneamente. El tiempo total 
de creación, ejecución y destrucción de los hilos se registra y exporta a un archivo CSV.
...
Compilación: gcc busq_exponencial_paralela.c tiempo.c -lpthread -o busq_exponencial_paralela
Ejecución: ./busq_exponencial_paralela <sucio.txt> <n> <hilos>
*/
...
void* busquedaExponencialHilo(void *arg);
...
int main(int argc, char *argv[]) {
    ...
    // 4. EJECUCIÓN DE BÚSQUEDA EXPONENCIAL PARALELA
    printf("\n[3/4] Ejecutando Busqueda Exponencial usando %d hilo(s)...\n", num_hilos);
    ...
    for (int j = 0; j < n_objetivos; j++) {
        ...
        for (int i = 0; i < num_hilos; i++) {
            ...
            pthread_create(&hilos[i], NULL, busquedaExponencialHilo, &args[i]);
        }
        ...
        fprintf(f_csv, "Busqueda Exponencial Paralela,%d,%d,%d,%d,%d,%.10f,%.10f,%.10f\n", 
                n, 
                buscado, 
                encontrado, 
                posicion_encontrada, 
...
/* --- FUNCIÓN DEL HILO (BÚSQUEDA EXPONENCIAL FRAGMENTADA) --- */

/*
void* busquedaExponencialHilo(void *arg)
DESCRIPCION: Función ejecutada por cada hilo. Comprueba si el objetivo pertenece matemáticamente 
al bloque asignado. Si es así, realiza la acotación exponencial adaptando los índices a su 
fragmento local, seguido de una búsqueda binaria. Aborta de inmediato si otro hilo reporta el hallazgo.
...
void* busquedaExponencialHilo(void *arg) {
    ...
    if (datos->objetivo >= datos->arreglo[datos->inicio] && 
        ...
        // Si el elemento está exactamente en la primera posición de este bloque
        if (datos->arreglo[inicio] == datos->objetivo) {
            *(datos->posicion_global) = inicio;
            return NULL;
        }

        // Saltos exponenciales relativos al inicio del bloque del hilo
        int i = 1;
        while (inicio + i <= fin && datos->arreglo[inicio + i] <= datos->objetivo) {
            if (*(datos->posicion_global) != -1) break; // Si otro hilo lo halló, abortar
            i = i * 2;
        }

        // Búsqueda binaria interna adaptada al rango encontrado
        int izquierda = inicio + i / 2;
        int derecha = (inicio + i <= fin) ? (inicio + i) : fin;
