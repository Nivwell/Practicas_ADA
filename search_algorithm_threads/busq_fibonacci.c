/*
IMPLEMENTACION DEL ALGORITMO DE BUSQUEDA DE FIBONACCI PARALELA (CON HILOS)
Autores: 
        Garcia Peñalva Saul 
...
DESCRIPCION: Este codigo implementa la Búsqueda de Fibonacci sobre un arreglo previamente 
ordenado con QuickSort, paralelizando el proceso mediante la biblioteca Pthreads. El arreglo 
se divide en bloques (chunks) asignados equitativamente a cada hilo. Para optimizar recursos, 
cada hilo comprueba primero si el objetivo se encuentra dentro del rango de valores de su bloque. 
De ser así, el hilo genera la secuencia de Fibonacci adecuada para la longitud de su subarreglo 
y realiza la búsqueda ajustando el 'offset' de forma local. Si otro hilo encuentra el objetivo, 
los demás abortan su ejecución. El tiempo total del ciclo de vida de los hilos se registra 
y exporta a un archivo CSV.
...
Compilación: gcc busq_fibonacci_paralela.c tiempo.c -lpthread -o busq_fibonacci_paralela
Ejecución: ./busq_fibonacci_paralela <sucio.txt> <n> <hilos>
*/
...
int min(int x, int y);
void* busquedaFibonacciHilo(void *arg);

/*
int main(int argc, char *argv[])
Recibe: argc - Número de argumentos recibidos.
        *argv[] - Vector de cadenas (argv[1] es el archivo desordenado, argv[2] es 'n', argv[3] es la cantidad de hilos).
...
Función: 
    ...
    5. Implementa el cronometraje abarcando el ciclo de vida completo de los hilos.
    6. Exporta los resultados (tiempo Real, User, Sys, posición y cantidad de hilos) al archivo CSV.
    7. Limpia la memoria dinámica antes de finalizar la ejecución.
*/
int main(int argc, char *argv[]) {
    ...
    // 4. EJECUCIÓN DE BÚSQUEDA DE FIBONACCI PARALELA
    printf("\n[3/4] Ejecutando Busqueda Fibonacci usando %d hilo(s)...\n", num_hilos);
    ...
    for (int j = 0; j < n_objetivos; j++) {
        ...
        for (int i = 0; i < num_hilos; i++) {
            ...
            pthread_create(&hilos[i], NULL, busquedaFibonacciHilo, &args[i]);
        }
        ...
        fprintf(f_csv, "Busqueda Fibonacci Paralela,%d,%d,%d,%d,%d,%.10f,%.10f,%.10f\n", 
                n, 
                buscado, 
                encontrado, 
                posicion_encontrada, 
...
/*
void* busquedaFibonacciHilo(void *arg)
DESCRIPCION: Función ejecutada por cada hilo. Comprueba si el objetivo pertenece al bloque 
asignado. De ser así, calcula la secuencia de Fibonacci necesaria para la longitud del bloque 
y realiza la búsqueda usando desplazamientos (offsets) relativos al índice inicial del hilo. 
Aborta su ejecución de inmediato si otro hilo ya encontró el elemento.
Recibe: arg - Puntero genérico (void*) a la estructura ThreadArgs con la configuración del bloque.
Devuelve: NULL al finalizar la ejecución del hilo.
*/
void* busquedaFibonacciHilo(void *arg) {
    ThreadArgs *datos = (ThreadArgs*)arg;
    
    // Optimización: Revisar si el número puede existir en el segmento del hilo
    if (datos->objetivo >= datos->arreglo[datos->inicio] && 
        datos->objetivo <= datos->arreglo[datos->fin]) {
        
        int len = datos->fin - datos->inicio + 1; // Longitud del bloque
        ...
        // fibM almacenará el número de Fibonacci más pequeño mayor o igual a la longitud del bloque
        while (fibM < len) {
            fibM2 = fibM1;
            fibM1 = fibM;
            fibM = fibM2 + fibM1;
        ...
        // El offset ahora se basa en el inicio del bloque asignado al hilo, no en cero absoluto.
        int offset = datos->inicio - 1;

        while (fibM > 1) {
            // Abortar si otro hilo ya lo encontró
            if (*(datos->posicion_global) != -1) break;

            int i = min(offset + fibM2, datos->fin);
            ...
            else if (datos->arreglo[i] > datos->objetivo) {
                fibM = fibM2;
                fibM1 = fibM1 - fibM2;
                fibM2 = fibM - fibM1;
            }
            else {
                *(datos->posicion_global) = i; // ¡Encontrado!
                break;
            }
        }

        // Comparar el último elemento con el objetivo
        if (*(datos->posicion_global) == -1 && fibM1 && offset + 1 <= datos->fin && datos->arreglo[offset + 1] == datos->objetivo) {
            *(datos->posicion_global) = offset + 1;
        }
    }
